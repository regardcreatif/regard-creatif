// ============================================================
// TRINITE CHAT — script.js v3 (splash fix + inscription manuelle)
// ============================================================
(function () {
  "use strict";

  // Force splash dismissal after 2 seconds max
  setTimeout(() => {
    const splash = document.getElementById("screen-splash");
    if (splash && splash.classList.contains("active")) {
      splash.style.display = "none";
      splash.classList.remove("active");
      console.log("Splash closed (timeout fallback)");
    }
  }, 2000);
...
// ============================================================
// AUTHENTIFICATION (avec fermeture IMMÉDIATE du splash)
// ============================================================
async function initAuth() {
  // Fermer splash IMMÉDIATEMENT
  const splash = document.getElementById("screen-splash");
  if (splash) {
    splash.style.display = "none";
    splash.classList.remove("active");
...
async function afterLogin() {
  ...
  try {
    ...
    if (!avatarBucketExists) console.warn("Bucket 'avatars' manquant");
  } catch (e) {}
...
document.getElementById("form-login")?.addEventListener("submit", async e => {
  ...
  if (error) toast(error.message.includes("Invalid") ? "Email ou mot de passe incorrect" : error.message, "error");
});
...
document.getElementById("form-register")?.addEventListener("submit", async e => {
  ...
  if (error) {
    ...
    const msg = error.message.includes("already registered") ? "Cet email est déjà utilisé." : error.message;
    toast(msg, "error");
    return;
  }
  ...
  if (user) {
    ...
    if (phone) await db.auth.updateUser({ phone, data: { phone } });
    const rows = [
    ...
    if (profileErr) console.warn("Profiles creation error:", profileErr.message);
  }

  const needsConfirm = !signUpData?.session;
  if (needsConfirm) toast("Compte créé ! Vérifiez votre boîte mail pour confirmer.", "success");
  else toast("Compte créé et connecté !", "success");
});
...
function buildSwiper() {
  ...
  swiperWrap?.addEventListener("touchstart", e => { startX = e.touches[0].clientX; startY = e.touches[0].clientY; dragging = false; }, { passive: true });
  ...
  swiperWrap?.addEventListener("touchend", e => {
    ...
    if (dx < -30 && activeIdx < userProfiles.length - 1) { setActiveProfile(activeIdx + 1); haptic(10); }
    else if (dx > 30 && activeIdx > 0) { setActiveProfile(activeIdx - 1); haptic(10); }
  });
}
...
async function setActiveProfile(idx) {
  ...
  if (wrap && wrap.children[0]) {
    const slideW = wrap.children[0].getBoundingClientRect().width;
    const gap = 12;
...
function wireSwipeDelete(wrapper, item, deleteBg, contact) {
  ...
  let currentX = 0;
  item.addEventListener("touchstart", e => { touchStartX = e.touches[0].clientX; currentX = 0; item.style.transition = "none"; }, { passive: true });
  ...
  item.addEventListener("touchend", () => {
    ...
    if (Math.abs(currentX) > 55) {
      ...
      deleteBg.addEventListener("click", async () => {
        ...
        else { toast("Contact supprimé", "info"); await loadContacts(); buildStories(); }
      }, { once: true });
    } else {
...
document.getElementById("contact-phone")?.addEventListener("input", () => {
  ...
  window._phoneSearchTimeout = setTimeout(() => {
    const phone = document.getElementById("contact-phone")?.value.trim();
    if (phone && phone.length >= 8) document.getElementById("btn-search-phone")?.click();
  }, 1000);
});
...
document.getElementById("form-add-contact")?.addEventListener("submit", async e => {
  ...
  const emailHidden = document.getElementById("contact-email").value.trim() || (phone.replace(/\s+/g,"") + "@phone.trinite");
...
// ============================================================
// ÉCRAN PROFIL (version simplifiée pour garder le fichier lisible)
// ============================================================
...
document.getElementById("btn-copy-id")?.addEventListener("click", () => {
  ...
  navigator.clipboard?.writeText(idText).then(() => toast("ID copié !", "success")).catch(() => toast("Impossible de copier", "error"));
});
...
// Avatar helpers (version simplifiée – garde les fonctions existantes mais on va les garder courtes pour la lisibilité)
...
async function uploadAvatar(file) {
  ...
  const { error: upErr } = await db.storage.from("avatars").upload(path, file, { cacheControl: "3600", upsert: true });
  if (upErr) { progressWrap?.classList.add("hidden"); toast("Erreur upload avatar : " + upErr.message, "error"); return; }
  if (progressBar) progressBar.style.width = "70%";
  const { data: urlData } = db.storage.from("avatars").getPublicUrl(path);
  const publicUrl = urlData?.publicUrl;
  if (!publicUrl) { progressWrap?.classList.add("hidden"); toast("Impossible de récupérer l'URL de l'avatar", "error"); return; }
  if (progressBar) progressBar.style.width = "90%";
  const myProfileIds = userProfiles.map(p => p.id);
  for (const pid of myProfileIds) await db.from("profiles").update({ avatar_url: publicUrl }).eq("id", pid);
...
document.getElementById("btn-avatar-remove")?.addEventListener("click", async () => {
  ...
  const myProfileIds = userProfiles.map(p => p.id);
  for (const pid of myProfileIds) await db.from("profiles").update({ avatar_url: null }).eq("id", pid);
  userAvatarUrl = null;
...
document.getElementById("btn-avatar-camera")?.addEventListener("click", async () => {
  try {
    ...
    haptic(10);
  } catch (err) { toast("Caméra inaccessible : " + err.message, "error"); }
});
...
function stopAvatarCamera() {
  if (avatarCamStream) { avatarCamStream.getTracks().forEach(t => t.stop()); avatarCamStream = null; }
  const video = document.getElementById("avatar-camera-stream");
  const controls = document.getElementById("avatar-camera-controls");
...
function initFab() {
  ...
  if (!fab) return;
  fab.addEventListener("click", () => { fabOpen = !fabOpen; toggleFabMenu(fabOpen); haptic(12); });
  document.querySelectorAll(".fab-option").forEach(btn => {
    btn.addEventListener("click", e => {
      e.stopPropagation();
      const type = btn.dataset.type;
      const idx = userProfiles.findIndex(p => p.profile_type === type);
      if (idx >= 0) { setActiveProfile(idx); toast(`Profil ${profileLabel(type)} activé`, "success"); }
      toggleFabMenu(false);
      haptic(15);
    });
  });
  document.addEventListener("click", e => { if (fabOpen && !e.target.closest("#fab-container")) toggleFabMenu(false); });
  wireFabDrag(fab);
}
...
function hideFab() { const container = document.getElementById("fab-container"); container?.classList.add("hidden"); toggleFabMenu(false); }
function updateFabVisibility(screenId) { const container = document.getElementById("fab-container"); if (!container) return; const hide = !screenId || screenId === "screen-auth" || screenId === "screen-setup"; container.classList.toggle("hidden", hide); }
function updateFabActiveState() { document.querySelectorAll(".fab-option").forEach(btn => { const isActive = activeProfile && btn.dataset.type === activeProfile.profile_type; btn.classList.toggle("active-profile", isActive); }); }
function wireFabDrag(fab) {
  let dragging = false, startX = 0, startY = 0, currentTarget = null;
  fab.addEventListener("touchstart", e => { startX = e.touches[0].clientX; startY = e.touches[0].clientY; dragging = false; currentTarget = null; }, { passive: true });
  fab.addEventListener("touchmove", e => {
    const dx = e.touches[0].clientX - startX;
    const dy = e.touches[0].clientY - startY;
    if (!dragging && Math.sqrt(dx*dx + dy*dy) > 12) { dragging = true; toggleFabMenu(true); }
    if (!dragging) return;
    e.preventDefault();
    ...
    document.querySelectorAll(".fab-option").forEach(btn => {
      ...
      if (dist < 55) { btn.style.transform = "scale(1.25)"; btn.style.boxShadow = "0 0 0 3px #fff, 0 0 24px rgba(255,255,255,0.5)"; }
  ...
  fab.addEventListener("touchend", () => {
    if (!dragging) return;
    dragging = false;
    document.querySelectorAll(".fab-option").forEach(b => { b.style.transform = ""; b.style.boxShadow = ""; });
    if (currentTarget) currentTarget.click();
    else toggleFabMenu(false);
    currentTarget = null;
  });
}
...
function openChat(contact, myProfile) {
  chatContact = contact; chatMyProfile = myProfile;
  ...
  if (avatarEl) { avatarEl.textContent = initial(contact.contact_name); avatarEl.style.background = "linear-gradient(135deg,var(--primary),var(--accent))"; }
  showScreen("screen-chat");
  const markAsRead = async () => { if (!chatContact.contact_profile_id) return; const { error } = await db.from('messages').update({ read_at: new Date().toISOString() }).eq('to_profile_id', chatMyProfile.id).eq('from_profile_id', chatContact.contact_profile_id).is('read_at', null); if (error) console.error("Erreur marquage lu:", error); };
  markAsRead();
  ...
  typingChannel.on('broadcast', { event: 'typing' }, (payload) => { const isTyping = payload.payload.isTyping; const indicator = document.getElementById('typing-indicator'); if (isTyping && indicator) indicator.classList.remove('hidden'); else if (indicator) indicator.classList.add('hidden'); }).subscribe();
  const input = document.getElementById('message-input');
  const handleTyping = () => { if (!chatMyProfile || !chatContact) return; typingChannel?.send({ type: 'broadcast', event: 'typing', payload: { userId: chatMyProfile.id, isTyping: true } }); clearTimeout(typingTimeout); typingTimeout = setTimeout(() => { typingChannel?.send({ type: 'broadcast', event: 'typing', payload: { userId: chatMyProfile.id, isTyping: false } }); }, 2000); };
...
async function loadMessages() {
  ...
  let query;
  if (contactPid) query = db.from("messages").select("*").or(`and(from_profile_id.eq.${myId},to_profile_id.eq.${contactPid}),and(from_profile_id.eq.${contactPid},to_profile_id.eq.${myId})`).order("created_at", { ascending: true }).limit(50);
  else query = db.from("messages").select("*").eq("from_profile_id", myId).order("created_at", { ascending: true }).limit(50);
  const { data, error } = await query;
  if (error) { toast("Erreur messages : " + error.message, "error"); return; }
  (data || []).forEach(msg => appendBubble(msg, myId));
  container.scrollTop = container.scrollHeight;
  if (!contactPid) { const hint = document.createElement("div"); hint.style.cssText = "text-align:center;font-size:0.75rem;color:var(--text-muted);padding:0.5rem 1rem;"; hint.textContent = "ℹ️ Ajoutez l'ID Trinite du contact pour voir les messages reçus."; container.insertBefore(hint, container.firstChild); }
}
function appendBubble(msg, myProfileId) {
  ...
  if (msg.content_type === "voice") div.innerHTML = `<div class="bubble-voice"><i class="fa-solid fa-microphone"></i><div class="bubble-voice-wave"><span></span><span></span><span></span><span></span><span></span><span></span></div></div><div class="bubble-voice-text">${escapeHtml(msg.content || "Message vocal")}</div><div class="bubble-time">${formatTime(msg.created_at)}</div>`;
  else { const isRead = msg.read_at !== null; const checkIcon = isRead ? '<i class="fa-solid fa-check-double"></i>' : '<i class="fa-regular fa-check"></i>'; div.innerHTML = `${escapeHtml(msg.content)}<div class="bubble-time">${formatTime(msg.created_at)} ${checkIcon}</div>`; }
...
function subscribeToMessages() {
  ...
  realtimeChannel = db.channel(channelName).on("postgres_changes", { event: "INSERT", schema: "public", table: "messages" }, (payload) => { const msg = payload.new; let relevant = false; if (contactPid) relevant = (msg.from_profile_id === myId && msg.to_profile_id === contactPid) || (msg.from_profile_id === contactPid && msg.to_profile_id === myId); else relevant = msg.from_profile_id === myId; if (relevant) appendBubble(msg, myId); }).subscribe();
}
function wireTypingIndicator() { const input = document.getElementById("message-input"); const indicator = document.getElementById("typing-indicator"); if (!input || !indicator) return; let typingTimeout = null; input.addEventListener("input", () => { clearTimeout(typingTimeout); typingTimeout = setTimeout(() => indicator.classList.add("hidden"), 3000); }); }
...
async function sendMessage() {
  ...
  haptic(8);
  const contactPid = chatContact.contact_profile_id || null;
  const { error } = await db.from("messages").insert({ from_profile_id: chatMyProfile.id, to_profile_id: contactPid, content, content_type: "text" });
...
document.getElementById("btn-back")?.addEventListener("click", () => {
  ...
  showScreen("screen-main");
  document.querySelectorAll(".nav-btn").forEach(b => b.classList.toggle("active", b.dataset.screen === "screen-main"));
});
...
async function toggleVoiceRecording() {
  ...
  try {
    ...
    mediaRecorder.onstop = async () => {
      ...
      const contactPid = chatContact.contact_profile_id || null;
      const { error } = await db.from("messages").insert({ from_profile_id: chatMyProfile.id, to_profile_id: contactPid, content: transcript, content_type: "voice" });
...
// ============================================================
// FEED TIKTOK (version simplifiée)
// ============================================================
async function buildFeed() {
  ...
  videos.forEach(v => { feedLikeCounts[v.id] = feedLikeCounts[v.id] ?? v.likes; });
  videos.forEach((v, index) => {
    const item = document.createElement("div");
    item.className = "feed-item";
    item.innerHTML = `<video class="feed-video" src="${escapeHtml(v.url)}" loop playsinline preload="none" ${feedSoundEnabled ? "" : "muted"}></video><div class="feed-item-gradient"></div>${v.isDemo ? "" : '<div class="feed-uploaded-badge">MES VIDÉOS</div>'}<div class="feed-item-info"><div class="feed-author">${escapeHtml(v.author)}</div><div class="feed-desc">${escapeHtml(v.desc)}</div></div><div class="feed-actions"><div style="display:flex;flex-direction:column;align-items:center;gap:0.3rem"><button class="feed-action-btn btn-like${feedLiked[v.id] ? " liked" : ""}" data-vid="${v.id}" aria-label="J'aime"><i class="fa-${feedLiked[v.id] ? "solid" : "regular"} fa-heart"></i></button><span class="feed-action-label liked-count">${formatCount(feedLikeCounts[v.id])}</span></div><div style="display:flex;flex-direction:column;align-items:center;gap:0.3rem"><button class="feed-action-btn btn-comment" aria-label="Commenter"><i class="fa-regular fa-comment"></i></button><span class="feed-action-label">${formatCount(v.comments)}</span></div><div style="display:flex;flex-direction:column;align-items:center;gap:0.3rem"><button class="feed-action-btn btn-share" aria-label="Partager"><i class="fa-solid fa-share"></i></button><span class="feed-action-label">Partager</span></div></div>`;
...
document.getElementById("btn-feed-sound")?.addEventListener("click", () => {
  ...
  document.querySelectorAll(".feed-video").forEach(v => v.muted = !feedSoundEnabled);
  haptic(10);
...
function wireFeedItem(item, video, index) {
  ...
  let lastTap = 0;
  item.addEventListener("touchend", e => { const now = Date.now(); if (now - lastTap < 280) { e.preventDefault(); toggleLike(video.id, item); } lastTap = now; });
  item.addEventListener("dblclick", () => toggleLike(video.id, item));
  let tapTimer = null;
  item.addEventListener("touchend", e => { if (tapTimer) clearTimeout(tapTimer); if (e.target.closest(".feed-actions")) return; tapTimer = setTimeout(() => { if (videoEl.paused) videoEl.play().catch(() => {}); else videoEl.pause(); }, 200); });
  item.querySelector(".btn-like")?.addEventListener("click", e => { e.stopPropagation(); toggleLike(video.id, item); haptic(15); });
  item.querySelector(".btn-comment")?.addEventListener("click", e => { e.stopPropagation(); openChatFromFeed(); });
  item.querySelector(".btn-share")?.addEventListener("click", e => { e.stopPropagation(); if (navigator.share) navigator.share({ title: "Trinite Chat", text: video.desc, url: location.href }); else toast("Lien copié !", "success"); });
  let touchStartX = null, touchStartY = null;
  item.addEventListener("touchstart", e => { touchStartX = e.touches[0].clientX; touchStartY = e.touches[0].clientY; }, { passive: true });
  item.addEventListener("touchend", e => { if (touchStartX === null) return; const dx = e.changedTouches[0].clientX - touchStartX; const dy = Math.abs(e.changedTouches[0].clientY - touchStartY); touchStartX = null; if (dx > 60 && dy < 60) openChatFromFeed(item); });
}
function toggleLike(videoId, item) {
  const wasLiked = feedLiked[videoId];
  feedLiked[videoId] = !wasLiked;
  if (!wasLiked) { feedLikeCounts[videoId]++; showHeartAnimation(item); showLikeParticles(item); haptic(15); } else { feedLikeCounts[videoId]--; }
  const btn = item.querySelector(".btn-like");
  const icon = btn?.querySelector("i");
  const likedCount = item.querySelector(".liked-count");
  if (btn) { btn.classList.toggle("liked", feedLiked[videoId]); if (icon) icon.className = `fa-${feedLiked[videoId] ? "solid" : "regular"} fa-heart`; if (likedCount) likedCount.textContent = formatCount(feedLikeCounts[videoId]); }
}
function showHeartAnimation(item) { const heart = document.createElement("div"); heart.className = "heart-anim"; heart.textContent = "❤️"; item.appendChild(heart); setTimeout(() => heart.remove(), 750); }
function showLikeParticles(item) { const colors = ["#fe2c55","#ff6b9d","#ff4081","#e91e8c","#f06292","#ff8a65","#ffd740"]; const count = 12; for (let i = 0; i < count; i++) { const angle = (i / count) * Math.PI * 2; const dist = 60 + Math.random() * 80; const tx = Math.cos(angle) * dist; const ty = Math.sin(angle) * dist; const duration = 0.6 + Math.random() * 0.4; const p = document.createElement("div"); p.className = "like-particle"; p.style.cssText = `left: calc(50% - 4px);top: calc(50% - 4px);background: ${colors[i % colors.length]};--tx: ${tx}px;--ty: ${ty}px;--particle-duration: ${duration}s;width: ${4 + Math.random() * 6}px;height: ${4 + Math.random() * 6}px;`; item.appendChild(p); setTimeout(() => p.remove(), duration * 1000 + 100); } }
function openChatFromFeed(item) { if (!activeProfile) { toast("Connectez-vous d'abord", "error"); return; } if (item) { const hint = document.createElement("div"); hint.className = "swipe-hint"; hint.textContent = "💬 Ouverture du chat…"; item.appendChild(hint); setTimeout(() => hint.remove(), 900); } setTimeout(() => { showScreen("screen-main"); document.querySelectorAll(".nav-btn").forEach(b => b.classList.toggle("active", b.dataset.screen === "screen-main")); pauseAllFeedVideos(); if (currentContacts.length > 0) openChat(currentContacts[0], activeProfile); else toast("Ajoutez un contact pour discuter !", "info"); }, 400); }
let feedObserver = null;
function initFeedObserver() { if (feedObserver) feedObserver.disconnect(); feedObserver = new IntersectionObserver((entries) => { entries.forEach(entry => { const videoEl = entry.target.querySelector(".feed-video"); if (!videoEl) return; if (entry.isIntersecting) { document.querySelectorAll(".feed-video").forEach(v => { if (v !== videoEl && !v.paused) v.pause(); }); videoEl.muted = !feedSoundEnabled; videoEl.play().catch(() => {}); } else { videoEl.pause(); } }); }, { root: document.getElementById("feed-container"), threshold: 0.6 }); document.querySelectorAll(".feed-item").forEach(item => feedObserver.observe(item)); }
function playCurrentFeedVideo() { const container = document.getElementById("feed-container"); if (!container) return; const items = Array.from(document.querySelectorAll(".feed-item")); const scrollTop = container.scrollTop; const h = container.clientHeight; let bestItem = null, bestOverlap = -1; items.forEach(item => { const top = item.offsetTop - scrollTop; const bottom = top + item.offsetHeight; const overlap = Math.min(bottom, h) - Math.max(top, 0); if (overlap > bestOverlap) { bestOverlap = overlap; bestItem = item; } }); if (bestItem) { const v = bestItem.querySelector(".feed-video"); if (v && v.paused) { v.muted = !feedSoundEnabled; v.play().catch(() => {}); } } }
function pauseAllFeedVideos() { document.querySelectorAll(".feed-video").forEach(v => { if (!v.paused) v.pause(); }); }

// ============================================================
// STUDIO (version minimale)
// ============================================================
...
const fileInput2 = document.getElementById("studio-file-input");
const btnUpload2 = document.getElementById("btn-studio-upload");
const cameraPreview2 = document.getElementById("studio-camera-preview");
const videoPreview2 = document.getElementById("studio-video-preview");
const photoPreview2 = document.getElementById("studio-photo-preview");
const placeholder2 = document.getElementById("studio-placeholder");
const uploadSection2 = document.getElementById("studio-upload-section");
const fileInfoEl2 = document.getElementById("studio-file-info");
const stCanvas2 = document.getElementById("studio-canvas");
btnCameraStart?.addEventListener("click", async () => { try { cameraStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment" }, audio: false }); cameraPreview2.srcObject = cameraStream; cameraPreview2.classList.remove("hidden"); videoPreview2.classList.add("hidden"); photoPreview2.classList.add("hidden"); placeholder2?.classList.add("hidden"); btnCameraStart.classList.add("hidden"); btnCameraStop.classList.remove("hidden"); btnTakePhoto.classList.remove("hidden"); if (uploadSection2) uploadSection2.style.display = "none"; studioFile = null; studioBlob = null; haptic(10); } catch (err) { toast("Caméra inaccessible : " + err.message, "error"); } });
btnCameraStop?.addEventListener("click", stopCamera);
function stopCamera() { if (cameraStream) { cameraStream.getTracks().forEach(t => t.stop()); cameraStream = null; } if (cameraPreview2) { cameraPreview2.srcObject = null; cameraPreview2.classList.add("hidden"); } btnCameraStart?.classList.remove("hidden"); btnCameraStop?.classList.add("hidden"); btnTakePhoto?.classList.add("hidden"); }
btnTakePhoto?.addEventListener("click", () => { if (!cameraStream || !stCanvas2) return; const vw = cameraPreview2.videoWidth || 640; const vh = cameraPreview2.videoHeight || 480; stCanvas2.width = vw; stCanvas2.height = vh; const c = stCanvas2.getContext("2d"); c.drawImage(cameraPreview2, 0, 0, vw, vh); stCanvas2.toBlob(blob => { if (!blob) return; studioBlob = blob; studioFileName = `photo_${Date.now()}.jpg`; const url = URL.createObjectURL(blob); photoPreview2.src = url; photoPreview2.classList.remove("hidden"); videoPreview2.classList.add("hidden"); placeholder2?.classList.add("hidden"); stopCamera(); showUploadSection2(`📷 Photo — ${(blob.size / 1024).toFixed(0)} Ko`); haptic(15); }, "image/jpeg", 0.88); });
fileInput2?.addEventListener("change", e => { const file = e.target.files?.[0]; if (!file) return; studioFile = file; studioBlob = null; studioFileName = file.name; const url = URL.createObjectURL(file); if (file.type.startsWith("video/")) { videoPreview2.src = url; videoPreview2.classList.remove("hidden"); photoPreview2.classList.add("hidden"); } else { photoPreview2.src = url; photoPreview2.classList.remove("hidden"); videoPreview2.classList.add("hidden"); } cameraPreview2.classList.add("hidden"); placeholder2?.classList.add("hidden"); stopCamera(); showUploadSection2(`📁 ${file.name} — ${(file.size / (1024*1024)).toFixed(2)} Mo`); e.target.value = ""; });
function showUploadSection2(info) { if (fileInfoEl2) fileInfoEl2.textContent = info; if (uploadSection2) uploadSection2.style.display = ""; }
btnUpload2?.addEventListener("click", async () => { const desc = document.getElementById("studio-desc")?.value.trim() || "Vidéo Trinite Chat"; const fileObj = studioBlob ? new File([studioBlob], studioFileName, { type: "image/jpeg" }) : studioFile; if (!fileObj) { toast("Aucun fichier à publier", "error"); return; } if (!currentUser) { toast("Connectez-vous d'abord", "error"); return; } const progressWrap = document.getElementById("studio-upload-progress"); const progressBar = document.getElementById("studio-progress-bar"); btnUpload2.disabled = true; btnUpload2.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Publication…'; progressWrap?.classList.remove("hidden"); if (progressBar) progressBar.style.width = "10%"; const ext = studioFileName.split(".").pop() || "mp4"; const path = `${currentUser.id}/${Date.now()}.${ext}`; const progInterval = setInterval(() => { const cur = parseFloat(progressBar?.style.width || "10"); if (cur < 85 && progressBar) progressBar.style.width = (cur + 5) + "%"; }, 300); const { error } = await db.storage.from("videos").upload(path, fileObj, { cacheControl: "3600", upsert: false, metadata: { uploader: activeProfile?.name || "user", description: desc } }); clearInterval(progInterval); btnUpload2.disabled = false; btnUpload2.innerHTML = '<i class="fa-solid fa-cloud-arrow-up"></i> Publier dans le Feed'; progressWrap?.classList.add("hidden"); if (progressBar) progressBar.style.width = "0%"; if (error) { toast("Erreur upload : " + error.message, "error"); return; } if (progressBar) progressBar.style.width = "100%"; toast("Publié dans le Feed ✓", "success"); haptic(20); studioFile = null; studioBlob = null; studioFileName = null; if (videoPreview2) { videoPreview2.src = ""; videoPreview2.classList.add("hidden"); } if (photoPreview2) { photoPreview2.src = ""; photoPreview2.classList.add("hidden"); } placeholder2?.classList.remove("hidden"); if (uploadSection2) uploadSection2.style.display = "none"; const descInput = document.getElementById("studio-desc"); if (descInput) descInput.value = ""; await buildFeed(); });
